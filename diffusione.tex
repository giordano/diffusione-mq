...
% Vedi Ballentine, Cohen (pagine 914-5, mi sembra spiegato bene, mette anche in
% relazione con la conservazione del numero di particelle che mi serve più
% avanti per il teorema ottico) o Gottfried.
...
\begin{figure}
  ...
  \\
  \subfloat[][\emph{Funzioni di Bessel sferiche di seconda specie}, o
  \emph{funzioni di Neumann sferiche}, degli ordini più
...
\begin{equation}
  ...
  \frac{\hslash^{2}k}{2mL} = \frac{\hslash^{2}}{2mL^{2}} kL. % = E/kL.
\end{equation}
% Nota: la derivazione (molto approssimata) di questa condizione è mia, però dà
% lo stesso risultato del Landau (vol. 3, pagina 199, equazione (45,6)) a meno
% di un fattore 2 ininfluente nella stima degli ordini di grandezza.  La
% dimostrazione del Landau sembra più generale però è un po' artificiosa, la mia
% dimostrazione è più semplice però ha un difetto: partendo dall'assunto che
% deve risultare $a << 1$, trovo che $a < b < c$ e deduco che deve aversi
% $c << 1$, cosa in realtà non necessariamente vera.  Tuttavia fra $a$, $b$ e
% $c$ c'è davvero poca differenza (la sostituzione |sin(x)/x| → 1/x per grandi
% valori di x non è malvagia e gli altri valori assoluti essenzialmente cambiano
% il segno globale) e comunque la dimostrazione non pretende di determinare un
% risultato esatto.
