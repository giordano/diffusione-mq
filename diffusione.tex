...
interazione.  Fisicamente possiamo interpretare il termine $\int gV\psi_{0}$ di
ordine $1$ come un singolo processo di diffusione, il termine
$\iint gVgV\psi_{0}$ di ordine $2$ come due processi di diffusione in sequenza,
ecc.  In questo contesto la funzione di Green è chiamata \emph{propagatore}
perché spiega come il disturbo prodotto dal potenziale si propaga fra
un'interazione e la successiva.  Feynman si è ispirato proprio alla serie di
Born per realizzare la sua formulazione della meccanica quantistica
relativistica espressa in termini di vertici ($V$) e propagatori ($g$).
% TODO: provare a disegnare la figura 11.13 del Griffiths e rivedere la forma di
% questo paragrafo.
...
\end{equation}
% TODO: fare la figura 11.12 del Griffiths
Poiché $\abs{\bm{k}} = \abs{\bm{k}'} = k$, il modulo $q$ del momento trasferito
vale $q = 2k\sin(\theta/2)$ e la dipendenza angolare dell'ampiezza di diffusione
è contenuta in $q$.
