...
\begin{equation}
  \begin{split}
    ...
    \psi_{3}(\bm{r}) + \cdots \\
    &=\psi_{0} + \frac{2 m}{\hslash^{2}}\int\dd^{3}\bm{r}_{0} G_{+}(\bm{r} -
    \bm{r}_{0}) V(r_{0}) \psi_{0}(\bm{r}_{0}) \\
    &+ \bigg(\frac{2m}{\hslash^{2}}\bigg)^{2} \int\dd^{3}\bm{r}_{0}G_{+}(\bm{r}
    - \bm{r}_{0}) V(r_{0})\int\dd^{3}\bm{r}_{1} G_{+}(\bm{r}_{0}
    -\bm{r}_{1}) V(r_{1}) \psi_{0}(\bm{r}_{1}) \\
    &+ \bigg(\frac{2m}{\hslash^{2}}\bigg)^{3} \int\dd^{3}\bm{r}_{0}G_{+}(\bm{r}
    - \bm{r}_{0}) V(r_{0})\int\dd^{3}\bm{r}_{1} G_{+}(\bm{r}_{0}
    -\bm{r}_{1}) V(r_{1}) \\
    &\cdot\int\dd^{3}\bm{r}_{2} G_{+}(\bm{r}_{1} -
    \bm{r}_{2}) V(r_{2}) \psi_{0}(\bm{r}_{2}) + \cdots.
  \end{split}
\end{equation}
...
ed è proprio in questi casi che è maggiormente utilizzata.
...
Introduciamo gli operatori risolventi, i corrispondenti delle funzioni di Green
nel linguaggio operatoriale, rispettivamente dell'hamiltoniana di particella
...
scrittura, nel seguito ometteremo l'operatore $\mathbb{I}$.  Introduciamo
...
\end{equation}
Vedremo che questo risultato è la generalizzazione della serie di Born nel
linguaggio operatoriale. %TODO: farlo vedere oppure togliere questa frase.
...
sferica \emph{uscente}.  È necessaria questa precisazione perché in alcuni
contesti può essere utile considerare funzioni d'onda $\psi^{(-)}$ il cui
comportamento asintotico è dato dalla somma di un'onda piana e un'onda sferica
\emph{entrante}.  Possiamo riscrivere l'equazione di Schröedinger precedente
anche come
\begin{equation}
...
e di una qualsiasi funzione $\ket{\phi_{a}}$ tale che
$(E - H_{0})\ket{\psi_{0}} = 0$, cioè $\ket{\psi_{0}}$ deve essere autostato
dell'hamiltoniana $H_{0}$ con lo stesso autovalore di energia di
$\ket{\psi_{a}^{(+)}}$.  Dunque abbiamo
\begin{equation}
  \label{eq:oper-lippman-schwinger}
...
L'equazione~\eqref{eq:oper-lippman-schwinger} contiene ``più informazione''
della semplice equazioni di Schröedinger~\eqref{eq:oper-schroedinger} perché
incorpora la condizione al contorno data dal termine $\ket{\psi_{0}}$.  Se
...
paragrafo~\ref{sec:equazione-ls}.  Il prodotto scalare fra due stati di
diffusione entrante e uscente $\braket{\psi_{b}^{(-)} | \psi_{a}^{(+)}}$
associati a due insiemi di numeri quantici $a$ e $b$ differenti (in particolare
sono diversi i vettori d'onda $\bm{k}_{a}$ e $\bm{k}_{b}$) definisce gli
elementi della cosiddetta \emph{matrice S} o \emph{matrice di diffusione}
\begin{equation}
...
autovalore $E_{b} = \hslash^{2}k_{b}^{2}/2m$.  Dall'equazione~\eqref{eq:baz}
abbiamo $G(z) = (1 + G(z)V)G_{0}(z)$, da cui
\begin{equation}
...
\end{equation}
Scriviamo l'equazione di Lippmann–Schwinger~\eqref{eq:oper-lippman-schwinger}
come
\begin{equation}
