...
% http://tex.stackexchange.com/q/59658#comment126261_59660
...
dell'hamiltoniana e dal momento che essa è indipendente dal tempo possiamo
considerare gli autostati stazionari.  In particolare siamo interessati agli
autostati con valori positivi dell'energia $E$, associati a stati non legati,
perché se lo stato delle due particelle fosse legato sarebbe nulla la
...
è di nostro interesse in quanto la sua hamiltoniana,
...
diffusione, lontano dall'influenza del potenziale di interazione, poiché negli
esperimenti i rivelatori vengono posti molto lontani dal centro diffusore.
...
relativa $r$ il potenziale $V(\bm{r})$ è praticamente nullo.  La funzione d'onda
conterrà un termine di onda piana del tipo $\e^{\uimm \bm{k}\cdot\bm{r}}$, con
$\bm{k}$ vettore d'onda associato alla particella di massa $m$ il cui modulo è
legato all'energia da $E = \hslash^{2}k^{2}/2m$.  Nella vicinanza del bersaglio,
la funzione d'onda della particella subirà una profonda modifica a causa
dell'interazione con il diffusore.  Tuttavia molto tempo dopo l'interazione,
...
d'onda trasmessa $\psi_{k,a}$ che continua a propagarsi nella direzione
$\versor{k}$, quindi ha la forma $\e^{\uimm \bm{k}\cdot\bm{r}}$, e una funzione
d'onda di diffusione $\psi_{k,b}(\bm{r})$
\begin{equation}
  \psi_{k}(\bm{r}) = \psi_{k,a}(\bm{r}) + \psi_{k,b}(\bm{r}).
\end{equation}
Nei pedici delle funzioni d'onda abbiamo evidenziato la dipendenza dall'energia
attraverso il numero d'onda $k$.
...
\begin{figure}
  ...
  \begin{tikzpicture}[font=\footnotesize]
    % onda piana prima della diffusione.  L'argomento del comando è il nome del
    % nodo (oppure le coordinate senza `()') in cui c'è la coda del vettore k
    \newcommand{\ondapiana}[1]{
      \foreach \x in {1, 1.25, 1.5}
        \draw ($(#1) + (\x,2)$) -- ++(0,-4);
      \draw[thick,-latex] (#1) -- ++(2.5,0) node[above] {$\bm{k}$};}

    %% prima della diffusione
    \coordinate (A) at (0,0); % inizio vettore k prima della diffusione
    \ondapiana{A} % onda piana
    \draw[decoration={brace,mirror},decorate] ($(A) - (0,2.25)$)
      -- node[below,align=center] {prima della \\ diffusione} ++(2.5,0);

    %% dopo la diffusione
    % onde sferiche
    \coordinate (B) at ($(A) + (8,0)$); % centro onde sferiche
    \foreach \x in {1.5, 1.75, 2} % onde sferiche
      \draw[dashed,thick] (B) circle (\x);
    \draw[thick,-latex] (B) node[left] {$O$} --
      ++($cos(45)*(2.5,0) + sin(45)*(0,2.5)$) node[above] {$\bm{k}'$};
    \draw[dotted] (B) node[dot] {} -- ++(1.25,0);
    \draw[-latex] ($(B) + (0.5,0)$) to[out=90,in=-45] node[right]
      {$\theta$} ($(B) + cos(45)*(0.5,0) + sin(45)*(0,0.5)$);
    \draw[dotted] ($(B) + cos(45)*(3.25,0) + sin(45)*(0,3.25)$) --
      ++($cos(45)*(1,0) + sin(45)*(0,1)$)
      node[above right] {$\bm{r}$} node[dot] {};

    % onda piana
    \coordinate (C) at ($(B) + (2.5,0)$); % inizio vettore k
    \ondapiana{C} % onda piana
    \draw[decoration={brace,mirror},decorate] ($(B) - (2,2.25)$) --
      node[below,align=center] {dopo la diffusione} ($(C) + (2.5,-2.25)$);
  \end{tikzpicture}
...
\end{equation}
Nei casi in cui la forma asintotica~\eqref{eq:forma-asintotica} della funzione
d'onda è valida, la sezione d'urto differenziale è calcolabile con la formula
precedente.  La sezione d'urto differenziale è la quantità di maggior interesse
nei problemi di diffusione perché è quella misurabile sperimentalmente e abbiamo
trovato che è uguale al modulo quadro dell'ampiezza di diffusione
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=0.7,font=\footnotesize]
    % comando per disegnare il pacchetto di onde piane.  L'argomento è il nome
    % del nodo (oppure le coordinate senza `()') in cui è centrato il pacchetto.
    \newcommand{\pacchettoonda}[1]{
      \draw[rounded corners,thick,fill=white] ($(#1) - (0.5,0.75)$)
        rectangle ($(#1) + (0.5,0.75)$);
      \foreach \x in {0.25, 0, -0.25}
        \draw ($(#1) - (\x,0.75)$) -- ++(0,1.5);
    }

    %% prima della diffusione
    \pacchettoonda{-13,0} % pacchetto di onde piane incidenti
    \draw[-latex,thick] (-12.25,0) -- ++(1,0);
    % centro diffusore
    \draw[thick] (-10,0) node[above left] {$O$} node[dot] {} circle (1);
    \draw[decoration={brace,mirror},decorate] (-13.5,-1.5)
      -- node[below,align=center] {prima della \\ diffusione} ++(4.5,0);

    %% dopo la diffusione
    % centro diffusore
    \draw[thick] (0,0) node[above left] {$O$} node[dot] {} circle (1);
    % onde sferiche
    \foreach \x in {3.5, 3.75, ..., 4.5}
      \draw[dashed,thick] (0,0) circle (\x);
    \pacchettoonda{4,0} % pacchetto di onde piane trasmesse
    \foreach \x in {0, 45, ..., 315}
      \draw[thick,-latex] ($cos(\x)*(4.75,0) + sin(\x)*(0,4.75)$) --
        ++($cos(\x)*(1,0) + sin(\x)*(0,1)$);
    \draw[dotted] (-2,0) -- ++(4,0);
    \draw[-latex] (1.5,0) to[out=90,in=-55] node[right]
      {$\theta$} ($cos(35)*(1.5,0) + sin(35)*(0,1.5)$);
    \draw[dotted] (0,0) -- ++($cos(35)*(5.75,0)+sin(35)*(0,5.75)$) node (A) {};
    \shadedraw[left color=white,right color=white!65!black,rotate=35]
      ($(A)-(0,0.35)$) rectangle ($(A)+(1.5,0.35)$) node[above] {rivelatore};
    \draw[decoration={brace,mirror},decorate] (-5.75,-6)
      -- node[below,align=center] {dopo la diffusione} ++(11.5,0);
  \end{tikzpicture}
  \caption{Cinematica della diffusione di un pacchetto di onde piane.  Prima
    della diffusione il pacchetto di onde piane si dirige verso il centro
    diffusore $O$.  Dopo la diffusione sono presenti un pacchetto di onde piane
    trasmesse e un pacchetto di onde sferiche uscenti diffuse dal potenziale.
    Questi due pacchetti interferiscono distruttivamente nella direzione in
    avanti ($\theta = 0$).}
  \label{fig:cin-diffusione-pacchetto}
\end{figure}
...
figura~\ref{fig:cin-diffusione-pacchetto}, e non riceve particelle trasmesse.
In questo modo si osservano solo pacchetti di onde diffuse, non è necessario
prendere in considerazione i termini di interferenza fra i due pacchetti e i
risultati sperimentali possono essere confrontati correttamente con la sezione
...
\end{equation}
con $U(r) = (2m/\hslash^{2})V(r)$.  L'hamiltoniana commuta con gli operatori di
momento angolare $\bm{L}^{2}$ e $L_{z}$ e sappiamo che una soluzione
...
con l'asse $\versor{z}$ (poniamo $\versor{z}$ tale che $\bm{k} = k\versor{z}$),
tuttavia non è presente alcuna dipendenza dall'angolo azimutale $\phi$ e ci sarà
pertanto simmetria cilindrica.  Per annullare la dipendenza della funzione
d'onda $\psi_{k}(\bm{r})$ da $\phi$ nello sviluppo in onde
...
dall'osservare che un'onda piana, in assenza di potenziale, con vettore d'onda
$\bm{k} = k\versor{z}$ può essere sviluppata in onde parziali nel seguente modo
(vedi~\textcite[928-929]{cohen:quantum-mechanics})
\begin{equation}
...
$\bm{k} = k\versor{z}$ e $\bm{r}$.  Supponiamo che il potenziale di interazione
sia a \emph{rapida decrescenza}, o \emph{a corto raggio}, vale a dire per $r$
tendente all'infinito va a $0$ più rapidamente di $1/r^{2}$
\begin{equation}
...
\begin{figure}
  \centering
  \begin{tikzpicture}[scale=0.7,font=\footnotesize]
    \draw[inner color=white!65!black,outer color=white!95!black](0,0)circle(4);
    \node[circle,draw,align=center,inner color=white!50!black,
      outer color=white!65!black,text=white] (0,0)
  ...
  \caption{Diffusione da potenziale a corto raggio con simmetria sferica.  Nelle
    ...
    distanza dal bersaglio, $kr \gg 1$, e sia il potenziale di interazione sia
    il potenziale centrifugo sono trascurabili rispetto a $k^2$.}
...
comportamento asintotico di $R_{kl}$ nella zona di radiazione è
\begin{equation}
...
nell'equazione~\eqref{eq:onde-parziali2}.  A questo punto confrontiamo
...
quello di introdurre uno sfasamento $\delta_{l}$ in ciascuna funzione radiale
asintotica $R_{kl}(r)$ dello sviluppo in onde
parziali~\eqref{eq:onde-parziali2}.
...
\begin{equation}
  ...
  \begin{split}
    ...
    &=\frac{4\pi}{k^{2}} \sum_{l}(2l+1)\sin^{2}\delta_{l} = \sum_{l} \sigma_{l},
  \end{split}
\end{equation}
con $\sigma_{l} = (4\pi/k^{2})(2l+1)\sin^{2}\delta_{l}$ componente parziale
$l$-esima della sezione d'urto totale.  Utilizzando le altre espressioni
dell'ampiezza di diffusione $f_{k}(\theta)$ si possono trovare con calcoli
analoghi espressioni differenti per $\sigma_{\textup{tot}}$
\begin{equation}
...
avanti $\theta=0$.  In questa direzione c'è la sovrapposizione e interferenza
tra il pacchetto di onde incidenti e onde diffuse elasticamente e questa causa
la rimozione di flusso dal fascio di particelle incidenti, come discusso alla
fine del paragrafo~\ref{sec:sez-urto-mq}.  La sezione d'urto totale quantifica
proprio questa rimozione e il teorema ottico rendo conto della conservazione del
flusso di probabilità totale.  L'aggettivo ``ottico'' è dovuto al fatto che in
ottica l'interferenza fra un'onda incidente e l'onda diffusa in avanti è
l'origine dell'ombra di un oggetto opaco.  Anche se abbiamo dimostrato il
teorema ottico solo per la diffusione elastica da potenziale sferico, esso vale
anche per processi di diffusione anelastica da altri tipi di potenziali.  In
...
che $R_{kl}(r)$ per $kr \ll l$ va come $(kr)^{l}$ e diventa sempre più piccolo
al crescere di $l$.  Se $U(r)$ è a corto raggio, in particolare con raggio
uguale a $L$, nell'equazione~\eqref{eq:diff-R-op} il potenziale è moltiplicato
per la piccola quantità $R_{kl}(r)$ e avrà un piccolo effetto sulla soluzione.
Questo rozzo ragionamento ci permette di intuire che gli sfasamenti $\delta_{l}$
sono piccoli se $kL \ll l$ e in questo caso la serie~\eqref{eq:serie-sfasamenti}
converge già dopo pochi termini e risulta maggiormente utile a livello pratico.
In molti casi già il termine con $l = 0$ è sufficiente a dare una buona stima
della sezione d'urto e in questi casi si parla di diffusione in onda $s$.  La
...
nel punto $r = L$.  La~\eqref{eq:diff-R-op} è un'equazione differenziale del
secondo ordine e ci saranno due soluzioni linearmente indipendenti, di cui solo
...
l'equazione precedente si ricava la tangente di $\delta_{l}$
\begin{equation}
...
derivate rispetto alla coordinata $r$, per le funzioni di Hankel si ha
...
$\lim_{r \to \infty} r^{2}V(\bm{r}) = 0$.  Inoltre vogliamo determinare
$\psi_{k}(\bm{r})$ molto lontano dalla regione di diffusione, dunque
$\abs{\bm{r}} \gg \abs{\bm{r}_{0}}$, e in questa zona utilizziamo le seguenti
...
L'ampiezza di diffusione~\eqref{eq:ampiezza-diffusione-op} si può approssimare
con
\begin{equation}
...
che l'ampiezza di diffusione esatta contenga tutte le potenze di $V$ nella serie
di Born, questo però solleva il problema della convergenza della serie, problema
...
Nel limite di basse energie, quindi piccoli valori $k$ e di $q$, possiamo porre
$\sin(qr)/q \approx r$ e l'ampiezza di diffusione di
...
sviluppo in onde parziali sono non trascurabili solo i primi termini, in accordo
con le considerazioni fatte alla fine del paragrafo~\ref{sec:sez-urto-op}.
...
\begin{equation}
  ...
  \begin{split}
    ...
    &=\psi_{0,k}(\bm{r}) + \frac{2 m}{\hslash^{2}}\int\dd^{3}\bm{r}_{0}
...
diffusore, cioè intorno al punto $\bm{r} = \bm{0}$ dove è più importante il
contributo di $V$ all'integrale, poiché lontano dall'origine è fuori
dall'influenza del potenziale e si comporta come un'onda sferica.  Pertanto
valutiamo
\begin{equation}
  \label{eq:foobar}
  \begin{split}
...
potenziali a corto raggio, tali che $\lim_{r \to \infty}r^{2}V(r) =
0$.\footnote{A
...
\begin{equation}
  ...
  \frac{\hslash^{2}}{mL^{2}}.
\end{equation}
La quantità $\hslash^{2}/mL^{2}$ ha un'interpretazione fisica: per il principio
di indeterminazione di Heisenberg essa è l'ordine di grandezza dell'energia
cinetica di una particella di massa $m$ confinata in un volume di dimensione
lineare $L$.  Quindi la condizione~\eqref{eq:condizione-born} equivale a dire
che l'approssimazione di Born è applicabile quando il potenziale (se attrattivo)
è sufficientemente debole da non creare uno stato legato per la particella
relativa.
...
l'integrale
\begin{equation}
  \int_{0}^{L} V(r_{0}) \frac{\e^{2\uimm kr_{0}}}{2\uimm} \dd r_{0} \sim
  \frac{V_{0}}{2\uimm} \int_{0}^{L} \e^{2\uimm kr_{0}} \dd r_{0} = V_{0}
  \frac{1 - \e^{2\uimm kL}}{4k}
\end{equation}
...
\begin{figure}
  ...
  \caption{Andamento in scala log-lin dei potenziali di Yukawa
    ($V_{\textup{Y}}(r) = \e^{-r}/r$) e di Coulomb ($V_{\textup{C}}(r)=1/r$).
    Il potenziale di Yukawa va rapidamente a zero, mentre quello di Coulomb ha
    una coda molto lunga.}
...
scambio di una particella massiva, successivamente identificata con il pione.
...
molti tipi di potenziali.  Innanzitutto $V_{0}$ compare al quadrato ed essa non
dipende dal fatto che il potenziale sia attrattivo o repulsivo.  Inoltre se
$k/\alpha \ll 1$ la sezione d'urto è approssimativamente isotropa e per
$k \to 0$ la sezione d'urto diventa indipendente dall'energia.  Invece
all'aumentare di $k$ la particella viene deflessa solo di angoli piccoli, la
...
Abbiamo più volte ripetuto che la teoria qui sviluppata non è applicabile, senza
...
Yukawa tende a quello di Coulomb nel limite $\mu \to 0$.  Possiamo provare a
calcolare la sezione d'urto coulombiana facendo tendere $\alpha$ a $0$ nella
formula~\eqref{eq:sezione-yukawa} e ponendo $V_{0} = Z_{1}Z_{2}e^{2}$
\begin{equation}
...
potenziale di Coulomb.  Nella pratica questo non si verifica e non si osserva la
divergenza della sezione d'urto totale perché p difficile sottoporre
effettivamente una particella a un singolo potenziale coulombiano.  È
...
risolvente considerato, l'operatore $z - H_{0}$ o $z - H$ non è invertibile.  Il
valore del risolvente nel campo dei reali può essere ottenuto mediante un
passaggio al limite dal campo dei complessi.  Introduciamo
l'\emph{operatore di trasferimento} $T$ definito dalla relazione
\begin{equation}
...
il secondo membro dell'equazione precedente può anche essere fattorizzato come
...
\begin{equation}
  T = V + VGV.
\end{equation}
Sostituendo la definizione~\eqref{eq:oper-trasferimento} dell'operatore di
trasferimento, oppure la serie~\eqref{eq:oper-G-serie}, nell'equazione
precedente e poi risolvendo iterativamente l'equazione risultante troviamo
\begin{equation}
  \label{eq:oper-T-serie}
  T = V + VG_{0}V + VG_{0}VG_{0}V + \cdots.
\end{equation}
...
$\bm{k}_{a}$, che caratterizzano lo stato.  Possiamo riscrivere l'equazione di
Schrödinger precedente anche come
\begin{equation}
...
del tipo: fascio incidente $+$ onda diffusa dal potenziale e con la stessa
energia del fascio incidente, come nella funzione d'onda
asintotica~\eqref{eq:forma-asintotica}.  Il risolvente $G_{0}(E^{+})$ è
associato a stati di diffusione $\ket{\psi^{(+)}}$ \emph{uscenti},
vedi~\textcite[450-452]{ballentine:quantum-mechanics}.  L'apice $(+)$ sta
proprio a indicare questo comportamento asintotico.  Un'altra possibile scelta
per il risolvente potrebbe essere
...
\begin{equation}
  \braket{\bm{r} | (E - H_{0})G_{0}(E^{\pm}) | \bm{r}'} = \braket{\bm{r} |
    \bm{r}'} \iff \bigg(\frac{\hslash^{2}k^{2}}{2m} +
  \frac{\hslash^{2}}{2m}\nabla^{2}\bigg) \braket{\bm{r} | G_{0}(E^{\pm}) |
    \bm{r}'} = \delta^{3}(\bm{r} - \bm{r}')
\end{equation}
...
matematicamente accettabile dell'equazione di Schrödinger e giochino un ruolo
importante nella teoria, essi sono fisicamente difficili da preparare, perché è
necessario mantenere la coerenza della funzione d'onda iniziale in una regione
...
quantici $a$ e $b$ (e in particolare ai vettori d'onda $\bm{k}_{a}$ e
$\bm{k}_{b}$) definisce gli elementi della cosiddetta \emph{matrice S} o
\emph{matrice di diffusione} o \emph{matrice di scattering}
\begin{equation}
...
\begin{equation}
  ...
  \braket{\psi_{0,\bm{k}_{b}} | V | \psi_{a}^{(+)}} = -\frac{2\pi\hslash^{2}}{m}
  f_{ab}^{(+)}(\Omega_{\bm{k}_{b}}),
\end{equation}
o, equivalentemente,
\begin{equation}
  ...
  f_{ab}^{(+)}(\Omega_{\bm{k}_{b}}) = -\frac{m}{2\pi\hslash^{2}}
  \braket{\psi_{0,\bm{k}_{b}} | T(E^{+}) | \psi_{0,\bm{k}_{a}}} =
  -\frac{m}{2\pi\hslash^{2}} \braket{\psi_{0,\bm{k}_{b}} | V | \psi_{a}^{(+)}}.
\end{equation}
...
nell'equazione~\eqref{eq:foobaz}.  L'ampiezza di diffusione di
Born~\eqref{eq:serie-ampiezza} poi si ricava sostituendo l'equazione precedente
nella~\eqref{eq:oper-amp-diff}.
