...
riferimento seguiti sono~\textcite{ballentine:quantum-mechanics} e
\textcite{cohen:quantum-mechanics}, in aggiunta ho integrato del materiale preso
da~\textcites{gottfried:quantum-mechanics} e
\textcite{griffiths:introduction-qm}.  Ogni errore presente in queste note è da
attribuire a me.
...
asintotico di $\psi_{k,b}(\bm{r})$.  Un'onda piana che urta contro un ostacolo,
cioè il bersaglio nel problema di diffusione, si diffonde in tutto lo spazio
come un'onda sferica uscente del tipo $f_{k}(\theta,\phi)\e^{\uimm kr}/r$, con
$f_{k}(\theta,\phi)$ ampiezza dell'onda di diffusione.  Essa dipende dalle due
coordinate angolari sferiche $\theta$ e $\phi$, perché la diffusione non è in
generale isotropica e dall'energia mediante il numero d'onda
...
% TODO: figura 8.1, pagina 348, del Gottfried.
...
% TODO: spiegare un po' meglio il teorema ottico, dire cosa fisicamente
% significa e che ha valenza generale, anche diffusioni anelastiche.
...
determinare $\psi_{k}(\bm{r})$ per $\abs{\bm{r}} \gg \abs{\bm{r}_{0}}$, possiamo
...
ambiguità.  L'ampiezza di diffusione di Born è quindi proporzionale alla
trasformata di Fourier del potenziale fatta rispetto al momento trasferito
$\bm{q}$.
...
\begin{equation}
  ...
  f_{k}(\theta) = -\frac{2m}{q\hslash^{2}}\int_{0}^{+\infty} rV(r)\sin(qr)\dd r.
\end{equation}
Il modulo $q$ del momento trasferito vale $q = 2k\sin(\theta/2)$, come mostrato
nella figura~\ref{fig:momento-trasferito}, e la dipendenza angolare e
dall'energia dell'ampiezza di diffusione è contenuta in $q$.  Poiché
l'approssimazione di Born è applicabile in caso di potenziale sufficientemente
debole, le deflessioni $\delta_{l}$ prodotte dal potenziale di interazione
sferico saranno molto piccole: $\delta_{l} \ll 1$.  In questo caso
...
\begin{equation}
  \begin{split}
    \psi_{k}(\bm{r}) &= \psi_{0,k}(\bm{r}) + \psi_{1,k}(\bm{r}) + \psi_{2,k}(\bm{r}) +
    \psi_{3,k}(\bm{r}) + \cdots \\
...
$\abs{\psi_{1,k}(\bm{r})} \ll 1$.  Valutiamo $\psi_{1,k}(\bm{r})$ nel punto
...
% TODO: fare il grafico dell'andamento di Yukawa e Coulomb
...
il raggio d'azione.  Questo potenziale fu introdotto da Yukawa negli anni Trenta
del Novecento per descrivere le interazioni nucleari fra protoni e neutroni
dovute allo scambio di un pione.  Il comportamento del potenziale di Yukawa
nell'origine e all'infinito soddisfa le ipotesi di applicabilità
dell'approssimazione di Born viste in precedenza.
...
\begin{equation}
  f_{k}(\theta) = -\frac{2mV_{0}}{q\hslash^{2}} \int_{0}^{+\infty} \e^{-\alpha
    r}\sin(qr)\dd r = -\frac{2mV_{0}}{\hslash^{2}(q^{2} + \alpha^{2})}.
\end{equation}
La sezione d'urto differenziale è
\begin{equation}
  \label{eq:sezione-yukawa}
  \sigma(\theta) = \frac{4m^{2}V_{0}^{2}}{\hslash^{4}(4k^{2}\sin^{2}(\theta/2) +
    \alpha^{2})^{2}}.
\end{equation}
...
$k/\alpha \ll 1$ la sezione d'urto è approssimativamente isotropica e nello
sviluppo in onde parziali sono non trascurabili solo i primi termini, come
potevamo aspettarci dalle considerazioni fatte nel
paragrafo~\ref{sec:sez-urto-op}.  Per $k \to 0$ la sezione d'urto diventa
indipendente dall'energia, mentre all'aumentare di $k$ diventa sempre più
piccata intorno alla direzione $\theta = 0$ e si comporta come $1/q^{4}$, o
$1/\theta^{4}$ per piccoli angoli.
% TODO: figura 8.2 di pagina 360 del Gottfried.
La sezione d'urto totale del potenziale di Yukawa è
\begin{equation}
...
al potenziale coulombiano.  Tuttavia osserviamo che l'interazione
...
solido si ottiene una divergenza, a causa dell'andamento $1/\sin^{4}(\theta/2)$
nell'origine.  Ciò è dovuto al fatto che il potenziale coulombiano non è a corto
raggio.  È interessante che la sezione per questo potenziale calcolata secondo
le regole della meccanica quantistica, sebbene per via
